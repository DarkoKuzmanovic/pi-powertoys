...
| **compact-model** | `/compact-keypool` | Manages the Gemini compaction key pool — a set of `google/gemini-3.1-flash-lite` API keys from separate Google Cloud projects that the compaction hook rotates across for adaptive failover. Masked fingerprint display, bullet-style key input (no plaintext in session/editor), per-key status (enabled/cooldown/daily-exhausted/quarantined), manual verification, and explicit MiniMax-M3 fallback when all keys are unavailable. |
| **speedtest** | `/speedtest` | Benchmarks the active model's TTFT, tokens/sec, and total latency. Supports Anthropic, OpenAI completions, OpenAI responses, OpenAI Codex Responses, and xAI Responses APIs. |
