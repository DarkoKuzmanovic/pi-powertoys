...
const FALLBACK_API_BASE = "https://generativelanguage.googleapis.com/v1beta/models";
const CACHE_PATH = dirname(new URL(import.meta.url).pathname) + "/contextual-working-cache.json";
...
const TOOL_MESSAGES: Record<string, string[]> = {
	...
	lsp_navigation: ["Navigating code...", "Jumping to definition...", "Resolving references..."],
	ast_search: ["Searching AST...", "Pattern matching...", "Structural search..."],
...
const DEFAULT_SEEDS = ["Dreaming electric", "Pondering existence", "Computing vibes", "Channeling thoughts"];
...
class MessageGenerator {
	...
	async generateMessage(toolName: string, toolInput?: Record<string, unknown>): Promise<string> {
		// Rate limit check — skip API if at limit
		if (!this.rateLimiter.canCall()) {
			throw new Error(`Rate limited — next call in ${Math.ceil(this.rateLimiter.timeUntilNextCall() / 1000)}s`);
		}

		// Deduplicate in-flight requests
		const cacheKey = `${toolName}:${toolInput ? Object.entries(toolInput).slice(0, 2).map(([k, v]) => `${k}=${String(v).slice(0, 30)}`).join("&") : ""}`;
	...
	private async fetchMessage(toolName: string, toolInput?: Record<string, unknown>): Promise<string> {
	...
	private async fetchFromPrimary(prompt: string, maxTokens = 20): Promise<string> {
	...
	private async fetchFromFallback(prompt: string, maxTokens = 20): Promise<string> {
...
function buildDynamicMessage(toolName: string, toolInput?: Record<string, unknown>): string {
	const messages = TOOL_MESSAGES[toolName];
	if (!messages) return "Working...";

	// Pick a random message for variety
	const base = messages[Math.floor(Math.random() * messages.length)] ?? "Working...";
...
function saveWorkingConfig(messageStyle: MessageStyle, indicator: IndicatorStyle): void {
...
export default function (pi: ExtensionAPI) {
	...
	const providerTag = () => messageStyle === "ai"
		? (process.env.OPENCODE_GO_API_KEY ? "deepseek-v4-flash" : process.env.GOOGLE_API_KEY ? "gemini" : "cache")
		: messageStyle;
	...
	const applyMessage = async (toolName: string, toolInput?: Record<string, unknown>, ctx?: ExtensionContext) => {
		let message: string;
				...
				if (!generator) {
					...
					try {
						message = await Promise.race([
							...
							new Promise<string>((resolve) => setTimeout(() => resolve(fallback), 2000)),
						]);
					} catch {
						message = fallback;
					}
	...
	pi.on("turn_end", async (event, ctx) => {
		...
		if (!name) {
			const words = userPrompt.replace(/[^\w\s-]/g, "").split(/\s+/).filter(Boolean);
	...
	pi.registerCommand("working-style", {
		...
		handler: async (args, ctx) => {
			...
			if (messageStyle === "ai" && !generator) {
				ctx.ui.notify("No OPENCODE_GO_API_KEY or GOOGLE_API_KEY — using cached/seed messages only.", "warning");
			}
	...
	pi.registerCommand("working-indicator", {
		...
		handler: async (args, ctx) => {
			...
			if (arg && !(arg in INDICATORS)) {
				ctx.ui.notify(`Usage: /working-indicator [${INDICATOR_NAMES.join("|")}]`, "error");
				return;
			}
			...
			if (arg) {
				...
				const picked = choice.replace(/ ✓$/, "").split(" ")[0]! as IndicatorStyle;
